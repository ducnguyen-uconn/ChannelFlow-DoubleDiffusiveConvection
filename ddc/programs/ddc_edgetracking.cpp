...
int EdgeStateTracking(FlowField& uL, FlowField& uH, FlowField& tempL, FlowField& tempH, FlowField& saltL, FlowField& saltH,
                      const EdgetrackingFlags& etflags, const AttractorFlags& aflags, const TimeStep& dt,
                      const DDCFlags& ddcflags, ostream& os);

void CreateFiles(const EdgetrackingFlags& etflags);
Real Bisection(FlowField& uL, FlowField& uH, FlowField& tempL, FlowField& tempH, FlowField& saltL, FlowField& saltH, Real t0,
               const EdgetrackingFlags& etflags, const AttractorFlags& aflags, const TimeStep& dt,
               const DDCFlags& ddcflags, ostream& os = std::cout);  // Return: lambdaH
bool AdvanceLH(FlowField& uL0, FlowField& uH0, FlowField& tempL0, FlowField& tempH0, FlowField& saltL0, FlowField& saltH0, int& t0,
               const EdgetrackingFlags& etflags, const TimeStep& dt, const DDCFlags& flags,
               const AttractorFlags& aflags, ostream& os, ConvergenceData& cd);
void ChooseNewLH(const FlowField& u0, const FlowField& temp0, const FlowField& salt0, Real& lambdaL, Real& lambdaH, const Real t,
                 const EdgetrackingFlags& etflags, const AttractorFlags& aflags, const TimeStep& dt,
                 const DDCFlags& ddcflags, ostream& os = std::cout);
void SaveForContinuation(const FlowField& uL, const FlowField& uH, const FlowField& tempL, const FlowField& tempH, const FlowField& saltL, const FlowField& saltH,
                         const string dirBisections, Real t);

Attractor CalcAttractor(const FlowField& u0, const FlowField& temp0, const FlowField& salt0, FlowField& returnUField, FlowField& returnTField, FlowField& returnSField,
                        const AttractorFlags& aflags, const TimeStep& dt, const DDCFlags& flags,
...
Attractor CalcAttractor(const FlowField& u0, const FlowField& temp0, const FlowField& salt0, const AttractorFlags& aflags, const TimeStep& dt,
                        const DDCFlags& flags, const string energyfile, const string saveFinalFF = "",
...
int main(int argc, char* argv[]) {
    cfMPI_Init(&argc, &argv);
    {
        ...
        ArgList args(argc, argv, "Find edge state in DDC");
        ...
        const string u_ifname = (cont) ? "" : args.getstr(3, "initial flowfield", "initial velocity flowfield");
        const string temp_ifname = (cont) ? "" : args.getstr(2, "initial flowfield", "initial temperature flowfield");
        const string salt_ifname = (cont) ? "" : args.getstr(1, "initial flowfield", "initial salinity flowfield");
        ...
        if (logfile == "stdout" || logfile == "cout") {
            ddcflags.logstream = &cout;
        } else {
            if (cfmpi->taskid() == 0)
                logstream.open((etflags.savedir + logfile).c_str());
            ddcflags.logstream = &logstream;
        }
        ostream& os = (ostream&)*ddcflags.logstream;
        ...
        stringstream sstr;
        sstr << ddcflags;
        printout("\nDDCFlags: " + sstr.str(), os);

        if ((ddcflags.symmetries.length() > 0) || (ddcflags.tempsymmetries.length() > 0) || (ddcflags.saltsymmetries.length() > 0)) {
            printout("Restricting flow to invariant subspace generated by symmetries", os);
            stringstream sstr2u, sstr2t, sstr2s;
            sstr2u << "Velocity: " << ddcflags.symmetries;
            sstr2t << "Temperature: " << ddcflags.tempsymmetries;
            sstr2s << "Salinity: " << ddcflags.saltsymmetries;
            printout(sstr2u.str(), os);
            printout(sstr2t.str(), os);
            printout(sstr2s.str(), os);
        }
        ...
        if (!cont) {
            ...
            salt = FlowField(salt_ifname, cfmpi);
            if (temp.Nd() != 1)
                cferror("Error initializing temp field: number of dimensions must be 1!");
            if (salt.Nd() != 1)
                cferror("Error initializing salt field: number of dimensions must be 1!");
            ...
            if (ddcflags.symmetries.length() > 0) {
                ...
                stringstream sstr2;
                sstr2 << ddcflags.symmetries;
                printout(sstr2.str(), os);
                u.project(ddcflags.symmetries);
            ...
            if (ddcflags.tempsymmetries.length() > 0) {
                ...
                stringstream sstr2;
                sstr2 << ddcflags.tempsymmetries;
                printout(sstr2.str(), os);
                temp.project(ddcflags.tempsymmetries);
                printout("Projection error == " + r2s(L2Dist(temp, tempH)));
            }
            saltH = salt;  // tmp for calculating projection error and optimizing FFTW
            if (ddcflags.saltsymmetries.length() > 0) {
                printout("Projecting Salinity FlowField to invariant subspace generated by symmetries", os);
                stringstream sstr2;
                sstr2 << ddcflags.saltsymmetries;
                printout(sstr2.str(), os);
                salt.project(ddcflags.saltsymmetries);
                printout("Projection error == " + r2s(L2Dist(salt, saltH)));
            }
            uH.optimizeFFTW(FFTW_PATIENT);
            tempH.optimizeFFTW(FFTW_PATIENT);
            saltH.optimizeFFTW(FFTW_PATIENT);

            fftw_savewisdom();
            if (chooseLHfirst) {
                ChooseNewLH(u, temp, salt, lambdaL, lambdaH, 0., etflags, aflags, dt, ddcflags, os);
            }
            uH = u;
            uH *= lambdaH;
            uL = u;
            ...
            string saltfileH = etflags.directoryBisections + "saltH" + r2s(t);
            string saltfileL = etflags.directoryBisections + "saltL" + r2s(t);
            printout("Loading flowfields from " + fffileL + " and " + fffileH, os);
            uH = FlowField(fffileH, cfmpi);
            uH.project(ddcflags.symmetries);
            uL = FlowField(fffileL, cfmpi);
            uL.project(ddcflags.symmetries);
            tempH = FlowField(tempfileH, cfmpi);
            tempH.project(ddcflags.tempsymmetries);
            tempL = FlowField(tempfileL, cfmpi);
            tempL.project(ddcflags.tempsymmetries);
            saltH = FlowField(saltfileH, cfmpi);
            saltH.project(ddcflags.saltsymmetries);
            saltL = FlowField(saltfileL, cfmpi);
            saltL.project(ddcflags.saltsymmetries);
            u = uH;
            uH.optimizeFFTW(FFTW_PATIENT);
            uH = u;
            ...
            if (etflags.tLastWrite >= ddcflags.T - 1) {
                cferror("Already over time limit");
            }
        }

        EdgeStateTracking(uL, uH, tempL, tempH, saltL, saltH, etflags, aflags, dt, ddcflags, os);
    }
    cfMPI_Finalize();
    return 0;
}
...
*/
int EdgeStateTracking(FlowField& uL_, FlowField& uH_, FlowField& tempL_, FlowField& tempH_, FlowField& saltL_, FlowField& saltH_,
                      const EdgetrackingFlags& etflags, const AttractorFlags& aflags, const TimeStep& dt,
                      const DDCFlags& ddcflags, ostream& os) {
    int t = ddcflags.t0;
    FlowField u, uL(uL_), uH(uH_);
    FlowField temp, tempL(tempL_), tempH(tempH_);
    FlowField salt, saltL(saltL_), saltH(saltH_);
    ...
    while (!finished && t <= ddcflags.T) {
        ...
        Bisection(uL, uH, tempL, tempH, saltL, saltH, t, etflags, aflags, dt, ddcflags, os);

        // Step 2: Advance FlowFields
        printout("t = " + i2s((int)t) + ": Advancing flowfields");
        bool finished = AdvanceLH(uL, uH, tempL, tempH, saltL, saltH, t, etflags, dt, ddcflags, aflags, os, convergencedata);
        if (!finished) {
            if (etflags.keepUL) {
                u = uL;
                temp = tempL;
            ...
            ChooseNewLH(u, temp, salt, lambdaL, lambdaH, t, etflags, aflags, dt, ddcflags, os);
            uL = u;
            uL *= lambdaL;
            uH = u;
            uH *= lambdaH;
...
void CreateFiles(const EdgetrackingFlags& etflags) {
    ...
    if (taskid == 0) {
        ...
        fL << ddcfieldstatsheader_t("t") << endl;
        fH << ddcfieldstatsheader_t("t") << endl;
        fL.close();
        fH.close();
...
Real Bisection(FlowField& uL, FlowField& uH, FlowField& tempL, FlowField& tempH, FlowField& saltL, FlowField& saltH, Real t0,
    ...
    SaveForContinuation(uL, uH, tempL, tempH, saltL, saltH, etflags.directoryBisections, t0);
    while (l2d > etflags.epsilonBisection) {
        ...
        if (nTraj == 1) {
            ...
            tempT += tempL;
            tempT *= 0.5;
            FlowField saltT = saltH;
            saltT += saltL;
            saltT *= 0.5;

            // Calculate attractor of ulambda and update lambaL or lambdaH accordingly
            string f = "";
            Attractor a = CalcAttractor(uT, tempT, saltT, aflags, dt, ddcflags,
            ...
            while (a == Laminar && j <= nTraj + 10) {
                ...
                saltT *= alpha;
                a = CalcAttractor(
                    uT, tempT, saltT, aflags, dt, ddcflags,
 ...
 * @return t, final time
 */
bool AdvanceLH(FlowField& uL0, FlowField& uH0, FlowField& tempL0, FlowField& tempH0, FlowField& saltL0, FlowField& saltH0, int& t,
               const EdgetrackingFlags& etflags, const TimeStep& dt, const DDCFlags& flags,
               const AttractorFlags& aflags, ostream& os, ConvergenceData& cd) {
    ...
    fieldsL[2] = saltL0;  // is saltL
    fieldsH[2] = saltH0;  // is saltH
    fieldsL[3] =
    ...
    DDC dnsH(fieldsH, flags);
    DDC dnsL(fieldsL, flags);
    ...
    psolver.solve(fieldsH[3], fieldsH[0]);
    psolver.solve(fieldsL[3], fieldsL[0]);
        ...
        if (stateIsOnEdge) {
            ...
            if (l2d < etflags.epsilonAdvance) {
                ...
                if (t > etflags.tLastWrite) {  // prevent writing the same stuff twice
                    printout(ddcfieldstats_t(fieldsH[0], fieldsH[1], fieldsH[2], t), fH);
                    printout(ddcfieldstats_t(fieldsL[0], fieldsL[1], fieldsL[2], t), fL);
                }
                ecf2 = ecf1;
                ecf1 = ecf0;
                ecf0 = Ecf(fieldsH[0]);
                ...
                if (saveIntervalReached) {
                    if (etflags.saveUH) {
                        ...
                        fieldsH[2].save(etflags.directoryFF + "sH" + i2s((int)t));
                    }
                    if (etflags.saveUL) {
                        fieldsL[0].save(etflags.directoryFF + "uL" + i2s((int)t));
                        fieldsL[1].save(etflags.directoryFF + "tL" + i2s((int)t));
                        fieldsL[2].save(etflags.directoryFF + "sL" + i2s((int)t));
                    }
                }
                if (ecfMinimum && etflags.saveMinima) {
                    if (etflags.saveUH) {
                        ...
                        fieldsH[2].save(etflags.directoryMinima + "sH" + i2s((int)t));
                    }
                    if (etflags.saveUL) {
                        fieldsL[0].save(etflags.directoryMinima + "uL" + i2s((int)t));
                        fieldsL[1].save(etflags.directoryMinima + "tL" + i2s((int)t));
                        fieldsL[2].save(etflags.directoryMinima + "sL" + i2s((int)t));
                    }
                }
                uL0 = fieldsL[0];
                uH0 = fieldsH[0];
...
void ChooseNewLH(const FlowField& u0, const FlowField& temp0, const FlowField& salt0, Real& lambdaL, Real& lambdaH, const Real t,
    ...
    while (AH != Turbulent) {
        ...
        salt = salt0;
        salt *= lambdaH;
        AH = CalcAttractor(u, temp, salt, aflags, dt, ddcflags,
    ...
    while (AL != Laminar) {
        ...
        AL = CalcAttractor(u, temp, salt, aflags, dt, ddcflags,
    ...
    tempH *= lambdaH;
    FlowField saltL(salt0);
    saltL *= lambdaL;
    FlowField saltH(salt0);
    saltH *= lambdaH;
    SaveForContinuation(uL, uH, tempL, tempH, saltL, saltH, etflags.directoryBisections, t);
}
...
void SaveForContinuation(const FlowField& uL, const FlowField& uH, const FlowField& tempL, const FlowField& tempH, const FlowField& saltL, const FlowField& saltH,
    ...
    string sffnameH = directoryBisections + "sH" + i2s((int)t);
    string sffnameL = directoryBisections + "sL" + i2s((int)t);
    saltH.save(sffnameH);
    saltL.save(sffnameL);
 ...
 */
Attractor CalcAttractor(const FlowField& u0, const FlowField& temp0, const FlowField& salt0, const AttractorFlags& aflags, const TimeStep& dt_,
                        const DDCFlags& flags, const string energyfile, const string saveFinalFF, const int tMin,
                        const int tMax, bool verbose, const int t0, ostream& os) {
    FlowField u(u0);
    FlowField temp(temp0);
    FlowField salt(salt0);
    return CalcAttractor(u0, temp0, salt0, u, temp, salt, aflags, dt_, flags, energyfile, saveFinalFF, tMin, tMax, verbose, t0, os);
}

Attractor CalcAttractor(const FlowField& u0, const FlowField& temp0, const FlowField& salt0,
                        FlowField& u, FlowField& temp, FlowField& salt,
                        const AttractorFlags& aflags, const TimeStep& dt_, const DDCFlags& flags,
    ...
    vector<FlowField> fields = {u, temp, salt, q};
    DDC dns(fields, flags);
    ...
    psolver.solve(fields[3], fields[0]);
    ofstream f;
    bool saveEnergy = (energyfile != "");
    if (saveEnergy) {
        if (u0.taskid() == 0)
            f.open(energyfile.c_str(), ios::out);
        printout(ddcfieldstatsheader_t("t"), f);
        printout(ddcfieldstats_t(u, temp, salt, t0), f);
    }
    ...
    while (result == NoAttractor && t < tMax) {
        ...
        if (saveEnergy) {
            printout(ddcfieldstats_t(fields[0], fields[1], fields[2], t + t0), f);
        }
